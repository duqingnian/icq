...
remove_stickers_pack_packet::remove_stickers_pack_packet(wim_packet_params _params, const int32_t _pack_id, std::string _store_id)
...
int32_t remove_stickers_pack_packet::init_request(const std::shared_ptr<core::http_request_simple>& _request)
{
    ...
    _request->push_post_parameter("product", store_id_);  // to fix infinite waiting in case of post request without params
...
int core::wim::remove_stickers_pack_packet::minimal_supported_api_version() const
{
