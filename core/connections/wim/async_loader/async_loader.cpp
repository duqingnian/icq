...
namespace
{
    ...
    core::log_replace_functor make_default_log_replace_functor(std::string_view _url)
    {
        ...
        f.add_url_marker(config::hosts::get_host_url(config::hosts::host_url_type::files_parse) + "/",
                         core::tail_from_last_range_evaluator(' '));
...
void core::wim::async_loader::download(download_params_with_default_handler _params)
{
    ...
    auto stop = [_params, cancelled = cancelled_tasks_]()
    {
        ...
        return cancelled->contains(_params.file_key_, _params.id_);
    };
    ...
    async_tasks_->run_async_function([wr_this = weak_from_this(), req = std::move(request), tmp_file_name = std::move(tmp_file_name), url = std::move(url), _params = std::move(_params), cancelled_tks = cancelled_tasks_]()
    {
        const auto start = std::chrono::steady_clock().now();

        req->get_async([wr_this, start, req, tmp_file_name = std::move(tmp_file_name), url = std::move(url), _params = std::move(_params), cancelled = std::move(cancelled_tks)]
        (curl_easy::completion_code _completion_code) mutable
        {
            ...
            cancelled->remove(_params.file_key_, _params.id_);
            ...
            log << "async request result:" << req->get_response_code() << (_completion_code == curl_easy::completion_code::success ? " (success)" : " (error)") << '\n';
            ...
            __INFO("async_loader",
                ...
                "handler  = <%2%>\n"
                "success  = <%3%>\n"
                "response = <%4%>\n", url% _params.handler_.to_string() % logutils::yn(_completion_code == curl_easy::completion_code::success) % req->get_response_code());

            req->get_response()->close();
            ptr_this->file_save_thread_->run_async_function([wr_this, _completion_code, req = std::move(req), params = std::move(_params), tmp_file_name = std::move(tmp_file_name), url = std::move(url)]() mutable
            {
                auto ptr_this = wr_this.lock();
                if (!ptr_this)
                    return 0;
                ...
                {
                    const auto response_code = req->get_response_code();
                    ...
                    {
                        default_data_t data(req->get_response_code(), req->get_header());
...
void core::wim::async_loader::cancel(file_key _key, std::optional<int64_t> _seq)
{
    if (_seq)
        cancelled_tasks_->add(_key, *_seq);
}
...
void core::wim::async_loader::download_file(download_params_with_info_handler _params)
{
   __INFO("async_loader",
    ...
    cancelled_tasks_->remove(_params.file_key_);
    ...
    async_tasks_->run_async_function([_params = std::move(_params), wr_this = weak_from_this(), local_handler]
    {
        ...
        if (ptr_this->cancelled_tasks_->remove(_params.file_key_, _params.id_))
        {
        ...
        else
        {
            ...
            params.file_key_ = std::move(_params.file_key_);
...
void core::wim::async_loader::download_image_metainfo(const std::string& _url, const wim_packet_params& _wim_params, link_meta_handler_t _handler, int64_t _id, std::string_view _log_str)
{
    ...
    download_metainfo(file_key(_url), signed_url, preview_proxy::parse_json, _wim_params, _handler, _id, "filesDownloadSnippetMetadata", std::move(params));
}

void core::wim::async_loader::download_file_sharing_metainfo(const core::tools::filesharing_id& _fs_id, const wim_packet_params& _wim_params, file_sharing_meta_handler_t _handler, int64_t _id)
{
    im_assert(!_fs_id.is_empty());

    auto signed_url = su::concat(urls::get_url(urls::url_type::files_info), _fs_id.file_id(), "/?f=json&ptt_text=1&aimsid=", _wim_params.aimsid_);
    if (const auto& source_id = _fs_id.source_id())
        signed_url += su::concat("&source=", *source_id);

    if (features::is_webp_original_accepted())
        signed_url += "&support_webp=1";

    auto log_params = make_filesharing_log_params(_fs_id);
    download_metainfo(file_key(_fs_id), signed_url, &file_sharing_meta::parse_json, _wim_params, _handler, _id, "filesDownloadMetadata", std::move(log_params));
}
    ...
    link_meta_handler_t _metainfo_handler, file_info_handler_t _preview_handler, int64_t _id)
{
    ...
    auto local_handler = link_meta_handler_t([_priority, _url, _wim_params, _metainfo_handler, _preview_handler, _id, wr_this = weak_from_this()](loader_errors _error, const link_meta_data_t& _data) mutable
    {
        auto ptr_this = wr_this.lock();
        if (!ptr_this)
            return;
        ...
        auto file_path = get_path_in_cache(ptr_this->content_cache_dir_, file_key(preview_url), path_type::link_preview);

        download_params_with_info_handler params(std::move(_wim_params));
        params.priority_ = _priority;
        params.url_ = std::move(preview_url);
        params.file_key_ = file_key(_url);
...
void core::wim::async_loader::download_image(priority_t _priority, const std::string& _url, const std::string& _file_name, const wim_packet_params& _wim_params, const bool _use_proxy, const bool _is_external_resource, file_info_handler_t _handler, int64_t _id)
{
    const auto path = _file_name.empty()
        ? tools::from_utf16(get_path_in_cache(content_cache_dir_, file_key(_url), path_type::file))
        : _file_name;
    ...
    priority_t _priority, const std::string& _contact, const std::string& _url, std::string _file_name, const wim_packet_params& _wim_params, int64_t _id, file_info_handler_t _handler)
{
    ...
    file_save_thread_->run_t_async_function<std::string>([content_cache_dir = content_cache_dir_, _url, file_name = std::move(_file_name)]() mutable
    {
        bool force_request_metainfo = true;
        const auto meta_path = get_path_in_cache(content_cache_dir, file_key(core::tools::filesharing_id::from_filesharing_uri(_url)), path_type::link_meta);
        if (tools::binary_stream json_file; json_file.load_from_file(meta_path))
        {
            if (const auto file_size = json_file.available(); file_size != 0)
            {
                ...
                auto meta_info = file_sharing_meta::parse_json(json.data());
                if (meta_info && meta_info->local_ && !meta_info->local_->local_path_.empty())
                {
                    ...
                    if (tools::system::is_exist(local))
                    {
                        ...
                        if (tools::system::get_file_size(local) == meta_info->info_.file_size_ && last_modified == meta_info->local_->last_modified_)
                        {
                            ...
                            if (file_name.empty())
                                file_name = meta_info->local_->local_path_;
                        }
                    }
                }
            }
        ...
        return file_name;
    })->on_result_ = [wr_this = weak_from_this(), _url, _wim_params, _priority, _contact, _handler = std::move(_handler), _id](std::string _file_name) mutable
    {
        auto ptr_this = wr_this.lock();
        if (!ptr_this)
            return;

        ptr_this->download_file_sharing_metainfo(core::tools::parse_new_file_sharing_uri(_url).value_or(core::tools::filesharing_id{}), _wim_params, file_sharing_meta_handler_t(
            [_priority, _contact, _url, _file_name, _wim_params, _handler = std::move(_handler), _id, wr_this](loader_errors _error, const file_sharing_meta_data_t& _data) mutable
        {
            ...
            features::cleanup_cache(ptr_this->content_cache_dir_);
            ...
            else
            {
                switch (content_type.type_)
                {
                    ...
                    case file_sharing_base_content_type::lottie:
                        file_path = get_path_in_cache(ptr_this->content_cache_dir_, file_key(_url), path_type::file) + tools::from_utf8(boost::filesystem::extension(meta->info_.file_name_));
                        break;
                    default:
...
void core::wim::async_loader::cancel_file_sharing(const std::string& _url, std::optional<int64_t> _seq)
{
    if (!_seq)
        return;
    std::scoped_lock lock(in_progress_mutex_);
    ...
    {
        cancel(file_key(_url), std::move(_seq));
        return;
    }
    if (auto it = in_progress_.find(_url); it != in_progress_.end())
...
std::wstring core::wim::async_loader::get_meta_path(const file_key& _file_key)
{
    return get_path_in_cache(content_cache_dir_, _file_key, path_type::link_meta);
}

void core::wim::async_loader::update_filesharing_meta(const std::wstring& _meta_path, std::function<void(file_sharing_meta_uptr&)> _meta_handler)
{
    ...
    if (json_file.load_from_file(_meta_path))
    {
        if (const auto file_size = json_file.available(); file_size != 0)
        {
            ...
            if (auto meta_info = file_sharing_meta::parse_json(json.data()))
            {
                _meta_handler(meta_info);
...
void core::wim::async_loader::save_filesharing_local_path(const std::wstring& _meta_path, const std::wstring& _path)
{
    update_filesharing_meta(_meta_path, [&_path](file_sharing_meta_uptr& meta_info)
    {
        if (!_path.empty())
...
void core::wim::async_loader::save_filesharing_antivirus_check_result(const std::wstring& _meta_path, core::antivirus::check::result _result)
{
    update_filesharing_meta(_meta_path, [_result](file_sharing_meta_uptr& meta_info)
    {
        meta_info->info_.antivirus_check_.result_ = _result;
    });
}
...
{
    return tools::from_utf16(get_path_in_cache(content_cache_dir_, file_key(_url), path_type::file));
}
...
core::wim::async_loader_log_params core::wim::async_loader::make_filesharing_log_params(const core::tools::filesharing_id& _fs_id)
{
    ...
    if (!_fs_id.is_empty())
    {
        params.log_functor_.add_url_marker(_fs_id.file_id(), distance_range_evaluator(-std::ptrdiff_t(_fs_id.file_id().size())));
        if (const auto source_id = _fs_id.source_id(); source_id && !_fs_id.source_id()->empty())
...
void core::wim::async_loader::download_file_sharing_impl(std::string _url, wim_packet_params _wim_params, downloadable_file_chunks_ptr _file_chunks, int64_t _id, std::string_view _normalized_url)
{
    ...
    async_tasks_->run_async_function([req = std::move(request), _url, _wim_params, _file_chunks, wr_this, _id]()
    {
        req->get_async([_url, _wim_params, _file_chunks, req, wr_this, _id](curl_easy::completion_code _completion_code) mutable
        {
            auto ptr_this = wr_this.lock();
            if (!ptr_this)
                return;

            const auto code = req->get_response_code();
            const auto success = _completion_code == curl_easy::completion_code::success;
            if (success && (code == 206 || code == 200 || code == 201))
            {
                const auto size = req->get_response()->all_size();
...
void core::wim::log_error(std::string_view _context, std::string_view _url, core::wim::load_error_type _type, bool _full_log)
{
    ...
    bs.write<std::string_view>(su::concat('\n', _context, ' ', get_error(_type), '\n'));
    if (_full_log)
